          ...
          >
            <div className="flex w-full flex-row justify-start gap-[20px] ">
              ...
              <div className="flex w-full flex-col items-start justify-between gap-2">
                <h5 data-testid="product-name" className="text-[20px] font-[600]">{product.name}</h5>
                <p data-testid="product-description" className="font-poly text-left italic">{product.description}</p>
                <div className="flex w-full items-center justify-between gap-2">
                  <div data-testid="product-attributes" className="font-[600] ">V, Ve, GF</div>
                  <div className="flex flex-row items-center gap-2">
                    <button data-testid="reduce-quantity" className="h-10 w-10 gap-3.5 rounded-md bg-[#E4EAEB]" onClick={() => changeCount(-1)}>
                      -
                    </button>
                    <div data-testid="quantity">{counter}</div>
                    <button data-testid="increase-quantity" className="h-10 w-10 gap-3.5 rounded-md bg-[#E4EAEB]" onClick={() => changeCount(1)}>
                      +
                    </button>
                  </div>
                </div>
